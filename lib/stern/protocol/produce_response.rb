module Stern
  module Protocol
    class ProduceResponse < Response
      ...
      def self.merge(*produce_responses)
        all_partition_offsets = []
        produce_responses.each do |offset_response|
      ...
      def self.read_partition_offsets(buffer, topic_name)
        ...
        buffer.read_array do
          ...
          if error_code == 0
            partitions[partition_id] = PartitionOffset.new(topic_name, partition_id, offset)
          else
            partitions[partition_id] = PartitionOffsetError.new(topic_name, partition_id, Errors[error_code].new)
          end
        end
        partitions
      end
